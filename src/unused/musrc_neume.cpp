...
void MusRC::DrawAncus( MusDC *dc, MusLayerElement *element, MusLayer *layer, MusStaff *staff )
{
    ...
    int bby = staff->m_y_drawing - m_doc->m_staffSize[staffSize];
    ...
    int ledge = m_doc->m_ledgerLine[staffSize][2];
    ...
    int ynn2 = ynn + (m_doc->m_halfInterl[staffSize])*((neume->m_pitches.at(1)).getPitchDifference());
    ...
    int ynn3 = ynn + (m_doc->m_halfInterl[staffSize])*((neume->m_pitches.at(2)).getPitchDifference());
...
void MusRC::DrawClivis( MusDC *dc, MusLayerElement *element, MusLayer *layer, MusStaff *staff ) 
{
	...
	neume->m_pitches.at(0).m_x_abs = xn;
    festa_string( dc, xn, ynn + 19, '3', staff, neume->m_cueSize);
    ynn2 = ynn + (m_doc->m_halfInterl[staffSize])*((neume->m_pitches.at(1)).getPitchDifference());
    if (ynn2 == ynn) {
        dx = CLIVIS_X_SAME;
...
void MusRC::DrawPorrectus( MusDC *dc, MusLayerElement *element, MusLayer *layer, MusStaff *staff ) 
{
    ...
    xn += 3*PUNCT_WIDTH + dx;
    int ynn2 = ynn + (m_doc->m_halfInterl[staffSize])*((neume->m_pitches.at(2)).getPitchDifference());
	neume->m_pitches.at(2).m_x_abs = xn;
    ...
    int ynn3 = ynn + (m_doc->m_halfInterl[staffSize])*((neume->m_pitches.at(1)).getPitchDifference());
...
void MusRC::DrawPorrectusFlexus( MusDC *dc, MusLayerElement *element, MusLayer *layer, MusStaff *staff)
{
	...
	neume->m_pitches.at(2).m_x_abs = xn;
    xn += PUNCT_WIDTH - 1;
    int ynn3 = ynn + (m_doc->m_halfInterl[staffSize])*((neume->m_pitches.at(3)).getPitchDifference());
...
void MusRC::DrawScandicusFlexus( MusDC *dc, MusLayerElement *element, MusLayer *layer, MusStaff *staff )
{
	...
	neume->m_pitches.at(2).m_x_abs = xn;
    xn += PUNCT_WIDTH - 2;
    int ynn4 = ynn + (m_doc->m_halfInterl[staffSize])*((neume->m_pitches.at(3)).getPitchDifference());
...
void MusRC::DrawTorculusResupinus( MusDC *dc, MusLayerElement *element, MusLayer *layer, MusStaff *staff )
{
	...
	neume->m_pitches.at(2).m_x_abs = xn;
    xn += PUNCT_WIDTH - 1;
    int ynn4 = ynn + (m_doc->m_halfInterl[staffSize])*((neume->m_pitches.at(2)).getPitchDifference());
...
void MusRC::DrawCompound( MusDC *dc, MusLayerElement *element, MusLayer *layer, MusStaff *staff ) 
{
    ...
    y1 = ynn + (m_doc->m_halfInterl[staffSize])*y1;
    y2 = ynn + (m_doc->m_halfInterl[staffSize])*y2;
...
void MusRC::DrawNeumeDots(MusDC *dc, MusLayerElement *element, MusLayer *layer, MusStaff *staff) 
{
	...
	for (vector<MusNeumeElement>::iterator i = neume->m_pitches.begin(); i != neume->m_pitches.end(); i++) {
		if (i->getOrnament() == DOT) {
			bool onspace = (i->m_y_drawing/(m_doc->m_halfInterl[staff->staffSize]))%2;
			int y = staff->m_y_drawing + i->m_y_drawing + m_doc->m_interl[staff->staffSize];
			y += (!onspace) ? (m_doc->m_halfInterl[staff->staffSize]) : 0;
			...
			y -= (noteabove) ? (m_doc->m_interl[staff->staffSize]) : 0;
...
void MusRC::DrawNeumeSymbol( MusDC *dc, MusLayerElement *element, MusLayer *layer, MusStaff *staff)
{
	...
	if ((symbol->getType() == NEUME_SYMB_FLAT) || (symbol->getType() == NEUME_SYMB_NATURAL)) {
		element->m_y_drawing = CalculateNeumePosY(staff, symbol->m_pname, layer->GetClefOffset(element), oct) + m_doc->m_interl[staff->staffSize];
	}
	...
	{
		element->m_y_drawing = -m_doc->m_staffSize[staff->staffSize] - m_doc->m_interl[staff->staffSize]*2;
	}
...
void MusRC::DrawNeumeClef( MusDC *dc, MusLayerElement *element, MusLayer *layer, MusStaff *staff)
{
	...
	int y = staff->m_y_drawing - m_doc->m_staffSize[staff->staffSize] - element->m_y_drawing + m_doc->m_halfInterl[staff->staffSize]; //with a fudge factor?
	std::string shape = nF_CLEF;
	
	switch (clef->getValue())
	{
		case nC1: shape = nC_CLEF;
		case nF1: y -= m_doc->m_interl[staff->staffSize]*3; break;
		case nC2: shape = nC_CLEF;
		case nF2: y -= m_doc->m_interl[staff->staffSize]*2; break;
		case nC3: shape = nC_CLEF; y -= m_doc->m_interl[staff->staffSize] - 1; break; 
		case nF3: y -= m_doc->m_interl[staff->staffSize] - 6; break;
...
void MusRC::DrawComma(MusDC *dc, MusLayerElement *element, MusStaff *staff, bool cueSize )
{
	int x = element->m_x_abs;
...
void MusRC::DrawFlat(MusDC *dc, MusLayerElement *element, MusStaff *staff, bool cueSize )
{
	int x = element->m_x_abs;
	int y = staff->m_y_drawing + element->m_y_drawing;
...
void MusRC::DrawNatural(MusDC *dc, MusLayerElement *element, MusStaff *staff, bool cueSize )
{
	int x = element->m_x_abs;
	int y = staff->m_y_drawing + element->m_y_drawing - m_doc->m_halfInterl[staff->staffSize]/2;
	festa_string(dc, x, y, nNATURAL, staff, cueSize);
}

void MusRC::DrawDivMinor(MusDC *dc, MusLayerElement *element, MusStaff *staff, bool cueSize )
{
	int x = element->m_x_abs;
	int y = staff->m_y_drawing + element->m_y_drawing + m_doc->m_halfInterl[staff->staffSize] - 3;
	festa_string(dc, x, y, nDIV_MINOR, staff, cueSize);
}

void MusRC::DrawDivMajor(MusDC *dc, MusLayerElement *element, MusStaff *staff, bool cueSize)
{
	int x = element->m_x_abs;
...
void MusRC::DrawDivFinal(MusDC *dc, MusLayerElement *element, MusStaff *staff, bool cueSize )
{
	int x = element->m_x_abs;
	int y = staff->m_y_drawing + element->m_y_drawing + m_doc->m_halfInterl[staff->staffSize] - 2;
...
void MusRC::DrawDivSmall(MusDC *dc, MusLayerElement *element, MusStaff *staff, bool cueSize )
{
	int x = element->m_x_abs;
	int y = staff->m_y_drawing + element->m_y_drawing;
