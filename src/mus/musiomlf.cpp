...
IMPLEMENT_DYNAMIC_CLASS(MusMLFSymbolWP, MusMLFSymbol)

//----------------------------------------------------------------------------
// MusMLFSymbolWP
//----------------------------------------------------------------------------

MusMLFSymbolWP::MusMLFSymbolWP( ) :
	MusMLFSymbol()
{
}

wxString MusMLFSymbolWP::GetLabel( )
{
	if ( m_type == 0 )
	{
		wxLogWarning( _("Uninitialized symbole") );
    ...
    MusFileOutputStream( file, filename )
{
	...
	wxASSERT_MSG( dict, "MusMLFOutput: dictionnary cannot be NULL");
	m_dict = dict;
}
...
MusMLFOutputWP::MusMLFOutputWP( MusFile *file, wxString filename, MusMLFDictionary *dict, wxString model_symbole_name ) :
    MusMLFOutput( file, filename, dict, model_symbole_name )
{
}


MusMLFOutputWP::MusMLFOutputWP( MusFile *file, int fd, wxString filename, MusMLFDictionary *dict, wxString model_symbole_name ) :
	MusMLFOutput( file, fd, filename, dict, model_symbole_name )
{
}


MusMLFOutputWP::~MusMLFOutputWP()
{

}


bool MusMLFOutputWP::WriteNote(  MusNote *note )
{
	int  code, oct;
	GetUt1( m_staff, note, &code, &oct);
...
bool MusMLFOutputWP::WriteSymbole(  MusSymbol *symbole )
{
	// gestion des segment de portees (pas actif ?????)
	if ((symbole->flag == BARRE) && (symbole->code == 'I'))
	{	
