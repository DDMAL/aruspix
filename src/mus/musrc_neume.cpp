...
void MusRC::DrawNeume( MusDC *dc, MusLaidOutLayerElement *element, MusLaidOutLayer *layer, MusLaidOutStaff *staff)
{
    ...
    element->m_yrel = CalculateNeumePosY(staff, neume->m_pname, layer->GetClefOffset( element ), oct);
	for (vector<MusNeumeElement>::iterator i = neume->m_pitches.begin(); i != neume->m_pitches.end(); i++) {
		i->dec_y = CalculateNeumePosY(staff, neume->m_pname + i->getPitchDifference(), layer->GetClefOffset(element), oct);
	}
...
void MusRC::DrawClivis( MusDC *dc, MusLaidOutLayerElement *element, MusLaidOutLayer *layer, MusLaidOutStaff *staff ) 
{
    ...
    DrawNeumeLedgerLines( dc, ynn2,bby,element->m_xrel + dx - 1,ledge, staffSize);
    festa_string( dc, xn + dx - 1, ynn2 + 19, nPUNCTUM, staff, neume->m_cueSize);
	neume->m_pitches.at(1).xrel = xn + dx - 1;
	if (ynn2 != ynn) {
...
{
	int x = element->m_xrel;
	int y = staff->yrel + element->m_yrel;
...
{
	int x = element->m_xrel;
	int y = staff->yrel + element->m_yrel + 6;
