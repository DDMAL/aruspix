...
void MusMLFDictionary::Load( TiXmlElement *file_root )
{
	...
	TiXmlElement *hmm = NULL;
	
	node = file_root->FirstChild( "dictionary" );
	if ( !node )
	{
    ...
    // staves
    for( node = root->FirstChild( "entry" ); node; node = node->NextSibling( "entry" ) )
    {
        elem = node->ToElement();
        if (!elem) 
			continue;
		...
		for( node = elem->FirstChild( "hmm" ); node; node = node->NextSibling( "hmm" ) )
		{
			hmm = node->ToElement();
			if (!hmm || !hmm->Attribute("name") || !hmm->Attribute("states")) 
				continue;
			
			word.m_hmms.Add( hmm->Attribute("name") );
			word.m_states.Add( atoi(hmm->Attribute("states")) );
		}
        m_dict.Add( word );
    }
}
