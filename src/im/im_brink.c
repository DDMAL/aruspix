...
int imProcessBrinkThreshold(const imImage* image, imImage* dest  )
{
	int i, j;
	int Topt = 0;
	...
	imCalcGrayHistogram(image, histo, NON_CUMULATIVE);  	// gray histogram computed
	...
	imProcessThreshold(image, dest, Topt, TRUE);
